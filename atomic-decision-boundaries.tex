...
We prove that no construction can make a split system equivalent to an atomic
system with respect to admissibility under all execution traces.  This
limitation is a structural property of the system architecture, not of policy
expressiveness or state availability.  We further formalize the \Escalate{}
...
admissible when evaluated may become inadmissible by execution time.  No policy,
regardless of its sophistication, can close this gap from within a split
architecture, because the gap is not an information problem---it is an
architectural one.
...
$\Dset = \{\Allow, \Refuse, \Escalate\}$.  The $\Escalate$ outcome, absent
from classical TOCTOU analyses~\cite{bishop96, tsafrir08}, corresponds to
suspension of the transition pending supervisor review---a first-class governance
action in modern agent systems~\cite{acp26}.  We show that $\Escalate$ does
not eliminate the atomicity requirement; it \emph{transfers} it to the
resolution step, and prove that the same structural gap arises there unless
resolution is itself atomic.
...
to enforcement-based monitoring.  Paper~3 (in preparation) addresses the fair
allocation of atomic governance decisions across agents operating under shared
...
the formal definition of admissibility from it.
...
\emph{consistent with} $\Adm$ if:
$D(s, a) = \Allow \Rightarrow \Adm(s, a)$ and
$D(s, a) = \Refuse \Rightarrow \neg\Adm(s, a)$.
\end{definition}
...
\begin{align}
  F(s, a) &= (d,\, s') \label{eq:atomic-pair}\\
  d &= D(s, a) \label{eq:atomic-decision}\\
  s' &= T(s, a) \quad \text{if } d = \Allow \label{eq:atomic-transition}\\
  s' &= s \quad \text{if } d = \Refuse \label{eq:atomic-refuse}
\end{align}
and the computation of $d$ and $s'$ corresponds to a \emph{single indivisible
transition} in the LTS: $s \xrightarrow{F(s,a)} s'$ with no intermediate
state between the evaluation of $d$ and the determination of $s'$.
\end{definition}
...
\times \{{\Allow, \Refuse}\} \to \hat{S}$ is defined by:
\begin{align}
...
We now prove the central result.  The theorem is stated for the general case;
the proof is constructive via the execution trace in Example~\ref{ex:violation}.
...
$T$.  Under Assumption~\ref{asm:nontrivial}, there exists an execution trace
...
\]
\begin{enumerate}
  \item The decision transition $\mathit{dec}(a)$ fires in state $s$.  Since
    $\Adm(s, a) = \mathsf{true}$ and $D$ is consistent with $\Adm$
    (Definition~\ref{def:decfun}), we have $D(s, a) = \Allow$.  The system
...
$F(s^*, a) = (\Refuse, s^*)$, since $\Adm(s^*, a) = \mathsf{false}$ and $D$
is consistent with $\Adm$.  No transition of $T$ fires.
...
$M_{\mathrm{atom}}$ does not.  No construction can equate the two systems
with respect to admissibility preservation under all traces, because any
construction that closes the gap for $\sigma^*$ must either (a) prevent
environment actions from interleaving between $\mathit{dec}(a)$ and
$\mathit{exec}(a)$---which is precisely the atomic boundary condition---or
(b) re-evaluate $D$ at execution time atomically with $T$---which is again
the atomic boundary condition.
\end{proof}
...
evaluates $D(s_t, a)$ in state $s_t$, and then fires $T$ separately.  By
Theorem~\ref{thm:main} applied to the resolution sub-process (with
$s_t$ as the initial evaluation state and a potential environment action $e'$
before $T$ fires), there exists a trace in which the supervisor's $\Allow$
is computed in $s_t$ but $T$ fires in $s^{**}$ with $\Adm(s^{**}, a) =
\mathsf{false}$.  Thus, $\mathrm{resolve}$ admits an inadmissible transition.
...
\midrule
RBAC~\cite{sandhu96} & Split & Permissions evaluated separately from execution \\
OPA~\cite{opa}        & Split & Policy evaluation external to transition \\
OPA + Redis           & Split & External state enriches $D$; gap remains (Cor.~\ref{cor:extstate}) \\
Kubernetes Admission  & Partially atomic & Webhook admits before object creation,
                                           but out-of-band \\
ACP~\cite{acp26}      & Atomic & Decision and state mutation jointly enforced \\
\bottomrule
\end{tabular}
\end{table}
...
Assumption~\ref{asm:nontrivial}, no system that separates evaluation from
execution can guarantee admissibility at execution time; only a system that
couples them as a single indivisible step at the decision boundary can provide
this guarantee.
...
(Paper~1) instantiates the atomic decision boundary for multi-agent environments.
The Invariant Measurement Layer~\cite{iml26} (Paper~2) operates above the
atomic enforcement layer to detect behavioral drift that remains invisible to
enforcement signals.  Paper~3 extends this framework to the fair allocation of
governance decisions across competing agents.  All three rest on the structural
...
\newblock Technical Report ESD-TR-73-51, USAF Electronic Systems Division,
  1972.

\bibitem{bishop96}
M.~Bishop and D.~Bailey.
\newblock A critical analysis of vulnerability taxonomies.
\newblock Technical Report CSE-96-11, University of California at Davis,
  1996.

\bibitem{cedar}
J.~Cutler et~al.
\newblock {Cedar}: A new policy language.
\newblock \emph{USENIX Security Symposium}, 2023.
...
\bibitem{acp26}
M.~Fernandez.
\newblock Agent control protocol: Admission control for agent actions.
\newblock arXiv:2603.18829 [cs.CR], DOI:10.5281/zenodo.19561638, 2026.

\bibitem{iml26}
M.~Fernandez.
\newblock From admission to invariants: Measuring deviation in delegated agent
  systems.
